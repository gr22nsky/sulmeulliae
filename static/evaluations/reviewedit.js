...
const fetchReviewDetails = async (reviewId) => {
    try {
        const response = await axios.get(`https://sulmeulliae.com/api/v1/evaluations/review/${reviewId}/`, {
            headers: {
...
document.getElementById("edit-review-form").addEventListener("submit", async function(event) {
    ...
    try {
        const response = await axios.put(`https://sulmeulliae.com/api/v1/evaluations/review/${reviewId}/`, {
            content: newContent,
            rating: newRating
        }, {
            headers: {
